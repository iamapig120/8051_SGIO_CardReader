...
uint8_x __at(0x03B0) Ep1Buffer[ENDP1_BUFFER_SIZE];                                                 // 端点1 IN缓冲区
uint8_x __at(ENDP0_BUFFER_SIZE + 2 * ENDP4_BUFFER_SIZE + ENDP1_BUFFER_SIZE) Ep2Buffer[2 * ENDP2_BUFFER_SIZE];                         // 端点2 OUT&IN缓冲区
uint8_x __at(ENDP0_BUFFER_SIZE + 2 * ENDP4_BUFFER_SIZE + ENDP1_BUFFER_SIZE + 2 * ENDP2_BUFFER_SIZE) Ep3Buffer[2 * ENDP3_BUFFER_SIZE]; // 端点3 OUT&IN缓冲区
// 自动分配地址从0x0200开始，需修改make文件
...
const uint8_c usbCfgDesc[] = {
    ...
    sizeof(SegaRepDesc), 0x00, //   wDescriptorLength[0] 69
...
void usbDevInit()
{
  ...
  UEP2_3_MOD = UEP2_3_MOD & ~bUEP2_BUF_MOD | bUEP2_RX_EN | bUEP2_TX_EN; // 端点2发送使能 128字节缓冲区
