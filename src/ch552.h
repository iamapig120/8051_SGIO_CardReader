...
typedef __bit               BOOL;
typedef __data uint8_t      uint8_d;
typedef __data uint16_t     uint16_d;
typedef __data uint32_t     uint32_d;
typedef __idata uint8_t     uint8_i;
typedef __idata uint16_t    uint16_i;
typedef __idata uint32_t    uint32_i;
typedef __xdata uint8_t     uint8_x;
typedef __xdata uint16_t    uint16_x;
typedef __xdata uint32_t    uint32_x;

typedef volatile uint8_t        uint8_v;
typedef __data uint8_v          uint8_dv;
typedef __xdata uint8_v         uint8_xv;
...
__sfr   __at (0xD0)         PSW;        // program status word
 ...
 __sbit __at (0xD5)         F0;         // bit addressable general purpose flag 0
 __sbit __at (0xD4)         RS1;        // register R0-R7 bank selection high bit
 __sbit __at (0xD3)         RS0;        // register R0-R7 bank selection low bit
...
//    11 - bank 3, R0-R7 @ address 0x18-0x1F
 ...
 __sbit __at (0xD1)         F1;         // bit addressable general purpose flag 1
 __sbit __at (0xD0)         P;          // ReadOnly: parity flag
...
#define CHIP_ID             SAFE_MOD    // ReadOnly: reading chip ID
...
__sfr   __at (0xA8)         IE;         // interrupt enable
 __sbit __at (0xAF)         EA;         // enable global interrupts: 0=disable, 1=enable if E_DIS=0
 __sbit __at (0xAE)         E_DIS;      // disable global interrupts, intend to inhibit interrupt during some flash-ROM operation: 0=enable if EA=1, 1=disable
 __sbit __at (0xAD)         ET2;        // enable timer2 interrupt
 __sbit __at (0xAC)         ES;         // enable UART0 interrupt
 __sbit __at (0xAB)         ET1;        // enable timer1 interrupt
 __sbit __at (0xAA)         EX1;        // enable external interrupt INT1
 __sbit __at (0xA9)         ET0;        // enable timer0 interrupt
 __sbit __at (0xA8)         EX0;        // enable external interrupt INT0
__sfr   __at (0xB8)         IP;         // interrupt priority and current priority
 __sbit __at (0xBF)         PH_FLAG;    // ReadOnly: high level priority action flag
 __sbit __at (0xBE)         PL_FLAG;    // ReadOnly: low level priority action flag
...
//    11 - unknown error
 __sbit __at (0xBD)         PT2;        // timer2 interrupt priority level
 __sbit __at (0xBC)         PS;         // UART0 interrupt priority level
 __sbit __at (0xBB)         PT1;        // timer1 interrupt priority level
 __sbit __at (0xBA)         PX1;        // external interrupt INT1 priority level
 __sbit __at (0xB9)         PT0;        // timer0 interrupt priority level
 __sbit __at (0xB8)         PX0;        // external interrupt INT0 priority level
__sfr   __at (0xE8)         IE_EX;      // extend interrupt enable
 __sbit __at (0xEF)         IE_WDOG;    // enable watch-dog timer interrupt
 __sbit __at (0xEE)         IE_GPIO;    // enable GPIO input interrupt
 __sbit __at (0xED)         IE_PWMX;    // enable PWM1/2 interrupt
 __sbit __at (0xEC)         IE_UART1;   // enable UART1 interrupt
 __sbit __at (0xEB)         IE_ADC;     // enable ADC interrupt
 __sbit __at (0xEA)         IE_USB;     // enable USB interrupt
 __sbit __at (0xE9)         IE_TKEY;    // enable touch-key timer interrupt
 __sbit __at (0xE8)         IE_SPI0;    // enable SPI0 interrupt
...
__sfr16 __at (0x8584)       ROM_ADDR;   // address for flash-ROM, little-endian
__sfr   __at (0x84)         ROM_ADDR_L; // address low byte for flash-ROM
__sfr   __at (0x85)         ROM_ADDR_H; // address high byte for flash-ROM
__sfr16 __at (0x8F8E)       ROM_DATA;   // data for flash-ROM writing, little-endian
...
__sfr   __at (0x90)         P1;         // port 1 input & output
 __sbit __at (0x97)         SCK;        // serial clock for SPI0
 __sbit __at (0x97)         TXD1;       // TXD output for UART1
 __sbit __at (0x97)         TIN5;       // TIN5 for Touch-Key
 __sbit __at (0x96)         MISO;       // master serial data input or slave serial data output for SPI0
 __sbit __at (0x96)         RXD1;       // RXD input for UART1
 __sbit __at (0x96)         TIN4;       // TIN4 for Touch-Key
 __sbit __at (0x95)         MOSI;       // master serial data output or slave serial data input for SPI0
 __sbit __at (0x95)         PWM1;       // PWM output for PWM1
 __sbit __at (0x95)         TIN3;       // TIN3 for Touch-Key
 __sbit __at (0x95)         UCC2;       // CC2 for USB type-C
 __sbit __at (0x95)         AIN2;       // AIN2 for ADC
 __sbit __at (0x94)         T2_;        // alternate pin for T2
 __sbit __at (0x94)         CAP1_;      // alternate pin for CAP1
 __sbit __at (0x94)         SCS;        // slave chip-selection input for SPI0
 __sbit __at (0x94)         TIN2;       // TIN2 for Touch-Key
 __sbit __at (0x94)         UCC1;       // CC1 for USB type-C
 __sbit __at (0x94)         AIN1;       // AIN1 for ADC
 __sbit __at (0x93)         TXD_;       // alternate pin for TXD of UART0
 __sbit __at (0x92)         RXD_;       // alternate pin for RXD of UART0
 __sbit __at (0x91)         T2EX;       // external trigger input for timer2 reload & capture
 __sbit __at (0x91)         CAP2;       // capture2 input for timer2
 __sbit __at (0x91)         TIN1;       // TIN1 for Touch-Key
 __sbit __at (0x91)         VBUS2;      // VBUS2 for USB type-C
 __sbit __at (0x91)         AIN0;       // AIN0 for ADC
 __sbit __at (0x90)         T2;         // external count input
 __sbit __at (0x90)         CAP1;       // capture1 input for timer2
 __sbit __at (0x90)         TIN0;       // TIN0 for Touch-Key
...
__sfr   __at (0xB0)         P3;         // port 3 input & output
 __sbit __at (0xB7)         UDM;        // ReadOnly: pin UDM input
 __sbit __at (0xB6)         UDP;        // ReadOnly: pin UDP input
 __sbit __at (0xB5)         T1;         // external count input for timer1
 __sbit __at (0xB4)         PWM2;       // PWM output for PWM2
 __sbit __at (0xB4)         RXD1_;      // alternate pin for RXD1
 __sbit __at (0xB4)         T0;         // external count input for timer0
 __sbit __at (0xB3)         INT1;       // external interrupt 1 input
 __sbit __at (0xB2)         TXD1_;      // alternate pin for TXD1
 __sbit __at (0xB2)         INT0;       // external interrupt 0 input
 __sbit __at (0xB2)         VBUS1;      // VBUS1 for USB type-C
 __sbit __at (0xB2)         AIN3;       // AIN3 for ADC
 __sbit __at (0xB1)         PWM2_;      // alternate pin for PWM2
 __sbit __at (0xB1)         TXD;        // TXD output for UART0
 __sbit __at (0xB0)         PWM1_;      // alternate pin for PWM1
 __sbit __at (0xB0)         RXD;        // RXD input for UART0
...
__sfr   __at (0x88)         TCON;       // timer 0/1 control and external interrupt control
 __sbit __at (0x8F)         TF1;        // timer1 overflow & interrupt flag, auto cleared when MCU enter interrupt routine
 __sbit __at (0x8E)         TR1;        // timer1 run enable
 __sbit __at (0x8D)         TF0;        // timer0 overflow & interrupt flag, auto cleared when MCU enter interrupt routine
 __sbit __at (0x8C)         TR0;        // timer0 run enable
 __sbit __at (0x8B)         IE1;        // INT1 interrupt flag, auto cleared when MCU enter interrupt routine
 __sbit __at (0x8A)         IT1;        // INT1 interrupt type: 0=low level action, 1=falling edge action
 __sbit __at (0x89)         IE0;        // INT0 interrupt flag, auto cleared when MCU enter interrupt routine
 __sbit __at (0x88)         IT0;        // INT0 interrupt type: 0=low level action, 1=falling edge action
...
__sfr   __at (0x98)         SCON;       // UART0 control (serial port control)
 __sbit __at (0x9F)         SM0;        // UART0 mode bit0, selection data bit: 0=8 bits data, 1=9 bits data
 __sbit __at (0x9E)         SM1;        // UART0 mode bit1, selection baud rate: 0=fixed, 1=variable
// SM0 & SM1: UART0 mode
...
//    11 - mode 3, 9-bit UART,     baud rate = variable by timer1 or timer2 overflow rate
 __sbit __at (0x9D)         SM2;        // enable multi-device communication in mode 2/3
#define MASK_UART0_MOD      0xE0        // bit mask of UART0 mode
 __sbit __at (0x9C)         REN;        // enable UART0 receiving
 __sbit __at (0x9B)         TB8;        // the 9th transmitted data bit in mode 2/3
 __sbit __at (0x9A)         RB8;        // 9th data bit received in mode 2/3, or stop bit received for mode 1
 __sbit __at (0x99)         TI;         // transmit interrupt flag, set by hardware after completion of a serial transmittal, need software clear
 __sbit __at (0x98)         RI;         // receive interrupt flag, set by hardware after completion of a serial receiving, need software clear
...
__sfr   __at (0xC8)         T2CON;      // timer 2 control
 __sbit __at (0xCF)         TF2;        // timer2 overflow & interrupt flag, need software clear, the flag will not be set when either RCLK=1 or TCLK=1
 __sbit __at (0xCF)         CAP1F;      // timer2 capture 1 interrupt flag, set by T2 edge trigger if bT2_CAP1_EN=1, need software clear
 __sbit __at (0xCE)         EXF2;       // timer2 external flag, set by T2EX edge trigger if EXEN2=1, need software clear
 __sbit __at (0xCD)         RCLK;       // selection UART0 receiving clock: 0=timer1 overflow pulse, 1=timer2 overflow pulse
 __sbit __at (0xCC)         TCLK;       // selection UART0 transmittal clock: 0=timer1 overflow pulse, 1=timer2 overflow pulse
 __sbit __at (0xCB)         EXEN2;      // enable T2EX trigger function: 0=ignore T2EX, 1=trigger reload or capture by T2EX edge
 __sbit __at (0xCA)         TR2;        // timer2 run enable
 __sbit __at (0xC9)         C_T2;       // timer2 clock source selection: 0=timer base internal clock, 1=external edge counter base T2 falling edge
 __sbit __at (0xC8)         CP_RL2;     // timer2 function selection (force 0 if RCLK=1 or TCLK=1): 0=timer and auto reload if count overflow or T2EX edge, 1=capture by T2EX edge
...
__sfr16 __at (0xCBCA)       RCAP2;      // reload & capture value, little-endian
...
__sfr   __at (0xF8)         SPI0_STAT;  // SPI 0 status
 __sbit __at (0xFF)         S0_FST_ACT; // ReadOnly: indicate first byte received status for SPI0
 __sbit __at (0xFE)         S0_IF_OV;   // interrupt flag for slave mode FIFO overflow, direct bit address clear or write 1 to clear
 __sbit __at (0xFD)         S0_IF_FIRST;// interrupt flag for first byte received, direct bit address clear or write 1 to clear
 __sbit __at (0xFC)         S0_IF_BYTE; // interrupt flag for a byte data exchanged, direct bit address clear or write 1 to clear or accessing FIFO to clear if bS0_AUTO_IF=1
 __sbit __at (0xFB)         S0_FREE;    // ReadOnly: SPI0 free status
 __sbit __at (0xFA)         S0_T_FIFO;  // ReadOnly: tx FIFO count for SPI0
 __sbit __at (0xF8)         S0_R_FIFO;  // ReadOnly: rx FIFO count for SPI0
...
__sfr   __at (0xC0)         SCON1;      // UART1 control (serial port control)
 __sbit __at (0xC7)         U1SM0;      // UART1 mode, selection data bit: 0=8 bits data, 1=9 bits data
 __sbit __at (0xC5)         U1SMOD;     // UART1 2X baud rate selection: 0=slow(Fsys/32/(256-SBAUD1)), 1=fast(Fsys/16/(256-SBAUD1))
 __sbit __at (0xC4)         U1REN;      // enable UART1 receiving
 __sbit __at (0xC3)         U1TB8;      // the 9th transmitted data bit in 9 bits data mode
 __sbit __at (0xC2)         U1RB8;      // 9th data bit received in 9 bits data mode, or stop bit received for 8 bits data mode
 __sbit __at (0xC1)         U1TI;       // transmit interrupt flag, set by hardware after completion of a serial transmittal, need software clear
 __sbit __at (0xC0)         U1RI;       // receive interrupt flag, set by hardware after completion of a serial receiving, need software clear
...
__sfr   __at (0x80)         ADC_CTRL;   // ADC control
 __sbit __at (0x87)         CMPO;       // ReadOnly: comparator result input
 __sbit __at (0x86)         CMP_IF;     // flag for comparator result changed, direct bit address clear
 __sbit __at (0x85)         ADC_IF;     // interrupt flag for ADC finished, direct bit address clear
 __sbit __at (0x84)         ADC_START;  // set 1 to start ADC, auto cleared when ADC finished
 __sbit __at (0x83)         CMP_CHAN;   // comparator IN- input channel selection: 0=AIN1, 1=AIN3
 __sbit __at (0x81)         ADC_CHAN1;  // ADC/comparator IN+ channel selection high bit
 __sbit __at (0x80)         ADC_CHAN0;  // ADC/comparator IN+ channel selection low bit
...
__sfr16 __at (0xC5C4)       TKEY_DAT;   // ReadOnly: touch-key data, little-endian
...
__sfr   __at (0xD8)         USB_INT_FG;     // USB interrupt flag
 __sbit __at (0xDF)         U_IS_NAK;       // ReadOnly: indicate current USB transfer is NAK received
 __sbit __at (0xDE)         U_TOG_OK;       // ReadOnly: indicate current USB transfer toggle is OK
 __sbit __at (0xDD)         U_SIE_FREE;     // ReadOnly: indicate USB SIE free status
 __sbit __at (0xDC)         UIF_FIFO_OV;    // FIFO overflow interrupt flag for USB, direct bit address clear or write 1 to clear
 __sbit __at (0xDA)         UIF_SUSPEND;    // USB suspend or resume event interrupt flag, direct bit address clear or write 1 to clear
 __sbit __at (0xD9)         UIF_TRANSFER;   // USB transfer completion interrupt flag, direct bit address clear or write 1 to clear
 __sbit __at (0xD8)         UIF_BUS_RST;    // bus reset event interrupt flag, direct bit address clear or write 1 to clear
...
__sfr16 __at (0xE5E4)       UEP2_DMA;   // endpoint 2 buffer start address, little-endian
...
__sfr16 __at (0xE7E6)       UEP3_DMA;   // endpoint 3 buffer start address, little-endian
...
__sfr16 __at (0xEDEC)       UEP0_DMA;   // endpoint 0 buffer start address, little-endian
...
__sfr16 __at (0xEFEE)       UEP1_DMA;   // endpoint 1 buffer start address, little-endian
...
typedef __xdata USB_HUB_DESCR *PXUSB_HUB_DESCR;
